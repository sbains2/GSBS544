...
print("Starting Round 2 model evaluation...")
...
poly_lasso_pipeline = Pipeline(steps=[
    ...
    ('regressor', Lasso(max_iter=10000, selection='random'))
])
...
poly_enet_pipeline = Pipeline(steps=[
    ('preprocessor', preprocessor),
    ('poly', PolynomialFeatures(degree=2, interaction_only=True, include_bias=False)),
    ('regressor', ElasticNet(max_iter=10000, selection='random'))
])
...
svr_param_grid = {
    'regressor__C': [10, 50, 100], 
    'regressor__epsilon': [0.01, 0.05, 0.1],
    'regressor__gamma': ['scale', 0.001, 0.01]
}
