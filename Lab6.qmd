...
# Creatting ggplot bar plot of the coefficients
(
...
Interpretation: The final model uses a lasso regression with the 5 most important numeric variables (Hits, RBI, AtBat, PutOuts) and their interactions with Division. Standardization followed the interaction expansion to balance the coefficient magnitudes. The top predictors in the final model include combinations such as standardized_hits, dummify_Division_W_standardized_AtBat, and standardized walks, each having a positive effects on salary. Overall, this indicates that higher hitting and offensive stats, especially for players in the East/West division, are the most influential in determining salary. Conclusively, we can see that regulaization improved the predictive stability while maintaining interpretability.