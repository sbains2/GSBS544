...
# Defining grid for the solver and shrinkage
# Since SVD doesn't support shrinkage, I used a list of dicts
LDAparams = [
    {'model__solver': ['svd']},
    {'model__solver': ['lsqr', 'eigen'], 'model__shrinkage': [0.1, 0.5, 0.9]}
]

# Running Grid Search
LDAgrid = GridSearchCV(LDApipe, LDAparams, cv=5, scoring='accuracy')
LDAgrid.fit(Xt, yt)

print(f'Best CV accuracy: {LDAgrid.best_score_}')
print(f'Best Params: {LDAgrid.best_params_}')


# Generating predictions on the validation set using the best model
best_lda = LDAgrid.best_estimator_
y_pred = best_lda.predict(Xv)
...
```
For the LDA model, we find that the mean cross validated metric for the best LDA model was .87
...
# Tuning the regularization params
qda_params = {
    'model__reg_param': [0.0, 0.1, 0.3, 0.5, 0.7, 0.9, 1.0]
}

# Running Grid Search
qdaGrid = GridSearchCV(QDApipe, qda_params, cv=5, scoring='accuracy')
# Fitting the pipe on the training data
qdaGrid.fit(Xt, yt)

print(f'Best CV accuracy: {qdaGrid.best_score_}')
print(f'Best Params: {qdaGrid.best_params_}')

# Generating predictions on the validation set using best model
best_qda = qdaGrid.best_estimator_
y_pred = best_qda.predict(Xv)
...
```
cross validated accuracy based on the best qda model: 0.87
...
```
cross validated accuracy: 0.6301
