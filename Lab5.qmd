...
# Making dummy variables for sex, region, smoking status - created but not used later
...
_The first plot that I made was to determine the individual medical costs for individuals that smoked. I used a boxplot to determine the distribution of charges given one's smoking status and found that those who smoked paid significantly more in individual medical costs. You're able to see that the median and IQR are much higher for smokers._
...
_The second plot I made was to determine the association between BMI and individual medical costs billed. One is able to see that charges rise with age. At higher BMI, there is greater dispersion between the charges._
...
_The third plot I made was to look at the distribution of individual medical costs between genders. While the interquartile ranges are somewhat similar, differences in sex appear to be less significant relative to smoking._


Part Two: Simple Linear Models
a. Construct a simple linear model to predict the insurance charges from the beneficiary’s age. Discuss the model fit, and interpret the coefficient estimates.

b. Make a model that also incorporates the variable sex. Report your results.

c. Now make a model that does not include sex, but does include smoker. Report your results.

d. Which model (Q2 or Q3) do you think better fits the data? Justify your answer by calculating the MSE for each model, and also by comparing R-squared values.
...
_In understanding the model fit for the predictor age on medical costs, we can see there is a weak association between the two variables with a R2 of .1287. In addition to this, there is a high model error indicated by the MSE value. In addition to this, we see that for every one year increase in age, there is a ~$214.55 increase in the individual costs billed by the health insurance._
...
d.
_From looking at the model metrics, we're able to see that age and smoking status tended to explain the variation of medical costs the most, with an R2 value of 0.831, because smoking is primary driver. In addition to this, we see that this model also has the lowest mean squared error. When looking at the root mean squared error for this model, we see that on average, the model prediction error is around $5,200._
...
# Defining target of medical cost and explanatory variable age, and age^2.
y = df['charges']
...
_With comparing the model metrics of individual medical costs dependent on age and age squared and individual medical costs dependent on age, we can see that there is a slightly lower R^2 score with the polynomial feature included. This means that a slightly lower portion of the variability in cost can be explained by the polynomial feature. In addition to this, the MSE is slightly higher with the polynomial feature included, indicating on average, there is a higher error rate._

c.
```{python}

# Defining target of medical cost and explanatory variable 4th degree polynomials.
y = df['charges']
...
# Defining target of medical cost and explanatory variable age up to the 12th degree polynomial.
y = df['charges']
...
_With comparing the model metrics of individual medical costs dependent on age and 12th degree polynomial of age and individual medical costs dependent on age, we can see that there is a slightly lower R^2 score with the 12th degree polynomial feature included. This means that a SLIGHTLY lower portion of the variability in cost can be explained by the polynomial feature. In addition to this, the MSE is slightly higher with the 12th degree polynomial feature included, indicating on average, there is a slightly higher error rate. An important consideration with the 12th degree polynomial is that they typically overfit and add little generalization compared to adding meaningful predictors like smoking status._
...
(
    ...
    + geom_point()
    + geom_line(aes(x='age', y='predictions_d12'), color='red')
    + labs(
        title='12th degree polynomial prediction'
    )
)
```
...
```
_With the new model for age as the only predictor based on fitting the model on the original data and running predictions for the new data, we see that the MSE is 136184222.69_
...
```
_With the new model for age and bmi as predictors based on fitting the model on the original data and running predictions for the new data, we see that the MSE is 133723221.99_
...
_With the new model with age, bmi, and smoker, based on fitting the model on the original data and running predictions for the new data, we see that the MSE is 35709591.85_
...
_With the new model with interaction term for age * smoker, and bmi * smoker, based on fitting the model on the original data and running predictions for the new data, we see that the MSE is 47916148.47_
...
e4_mse = mean_squared_error(part4['charges'], y_pred_best)
rmse = math.sqrt(e4_mse)

print(f"R2: {r2}, MSE: {e4_mse}, RMSE: {rmse}")
...
# Creating a new column for the predicted fitted model
part4['fitted'] = y_pred_best


# Creating new column for the residuals
part4['residuals'] = part4['charges'] - part4['fitted']

(
    ggplot(part4, aes('fitted', 'residuals'))
    + geom_point()
...
print('Improvement in model versus part 4:', e4_mse - mse)

```
We're able to see that from the positive value by subtracting the explored model's MSE from part 4 model's MSE, part 5 model's has a lower MSE, indicating on average that there's a lower model error rate.


Plotting the residuals
```{python}
# Making a plot showing the residuals for the best model, with the full interaction
# Formatting the final fitted predictions into column for neatness
part4['fitted_final'] = y_pred_final
# Creating new column for the residuals
part4['residuals'] = part4['charges'] - part4['fitted_final']

(
    ggplot(part4, aes('fitted_final', 'residuals'))
    + geom_point()
    + geom_hline(yintercept=0)
    + labs(
        x='Fitted values of the final model'
    )
)
...
Conclusion:
The residual plot shows that most points are centered around 0, indicating that it is a reasonably good fit. However, at higher fitted values, there is more variation and spread in the residuals. Overall, the model explains the data accurately, but can be further refined to handle the larger charges more effectively.