...
_The first plot that I made was to determine the individual medical costs for individuals that smoked. I used a boxplot to determine the distribution of mcharges given one's smoking status and found that those who smoked paid significantly more in individual medical costs._
...
_With comparing the model metrics of individual medical costs dependent on age and 12th degree polynomial of age and individual medical costs dependent on age, we can see that there is a significantly lower R^2 score with the polynomial feature included. This means that a lower portion of the variability in cost can be explained by the polynomial feature. In addition to this, the MSE is higher with the polynomial feature included, indicating on average, there is a higher error rate._
...
f. 
```{python}
# Subsetting dataframe to age, charges, and 12th degree polynomial age
plotting = df[['age', 'charges', 'age_d12']]
# Making predictions for 12th degree age polynomial for series
plotting['predictions_d12'] = m.predict(plotting[['age', 'age_d12']])
plotting = plotting.sort_values('age')

(
    ggplot(plotting, aes(x='age', y='charges'))
    + geom_point()
...
X = df[['age_smoker', 'bmi_smoker', 'age', 'bmi', 'smoker_int']]
...
# using the fitted model to predict on the new data
y_pred = m.predict(part4[['age_smoker', 'bmi_smoker', 'age', 'bmi', 'smoker_int']])
mse = mean_squared_error(part4['charges'], y_pred)
rmse = math.sqrt(mse)
